<template>
    ...
    <el-card style="margin: 10px 0px;">
       ...
       <el-popconfirm :title="`确定删除?`" width="260px" @confirm="deleteSelectUser">
            <template #reference>
                <el-button type="primary" size="default" :disabled="selectIdArr.length ? false : true">批量删除</el-button>
            </template>
        </el-popconfirm>>
        <!-- table展示用户信息 -->
    ...
    <el-drawer v-model="drawer1" @open="checkAllToZ">
        <template #header>
            <h4>分配角色(职位)</h4>
        </template>
        <template #default>
...
const updateUser = (row: User) => {
    //抽屉显示出来
    if(userStore.$state.username==row.username){
        ElMessage({ type: 'error', message: '不能修改自己用户名'});
        return ;
    }
    drawer.value = true;
    //存储收集已有的账号信息
...
const deleteUser = async (row:any) => {
    if(userStore.$state.username==row.username){
        ElMessage({ type: 'error', message: '不能删除自己用户'});
        return ;
    }
